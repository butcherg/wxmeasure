...
wxArrayString wxsplit(wxString str, wxString delim)
{
	wxArrayString a;
	...
	for (std::map<std::string, std::string>::iterator it=props.begin(); it!=props.end(); ++it) {
		...
		if (tplate != std::string()) {
			...
			if (tplate.find_first_of("|") != std::string::npos) {
				wxArrayString choices = wxsplit(wxString(tplate.c_str()), "|");
				...
				//wxArrayString parms = wxsplit(wxString(template.c_str()), ":");
